...
Jbool UtilsSHA1WithOpenSSL::sha1Ready()
{
    SHA1_Init(&this->mCtx);
    return true;
}

Jbool UtilsSHA1WithOpenSSL::sha1Process(const Jbyte *v, Jint vLen)
{
    SHA1_Update(&this->mCtx, v, vLen);
    return true;
}

Jbool UtilsSHA1WithOpenSSL::sha1Done(std::vector<Jbyte> &ret)
{
    Jint i = 0;
